...
// create particle group/explosion
    ...
    function drawText(ctx, reveal)
    {
        var centerX = canvas.width / 2;
        var centerY = canvas.height / 2;

        ctx.font= "48px serif";
        var text = "#" + winner.item + " - " + winner.prizeName;
        console.log(text);
        ctx.fillStyle = 'white';
        setFontSize(text);
        var size = ctx.measureText(text);
        ctx.fillText(text, centerX - size.width / 2, centerY /2 );
        ...
        if(reveal === true) {
            ctx.fillStyle = 'white';
            setFontSize(text);
            ctx.fillText(text, centerX - size.width / 2, centerY + centerY /2 );
        }
    }
    ...
    function loop() {
        ...
        if( limiterTick >= limiterTotal ) {
            if( mousedown ) {
                ...
                fireworks.push( new Firework( cnw / 2, ch, mx, my ) );
                limiterTick = 0;
            }
        } else {
            limiterTick++;
